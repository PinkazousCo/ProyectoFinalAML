...
class DropColumns(BaseEstimator, TransformerMixin):
    ...
    def preprocess(data):
        ...
        data = data.drop(drop_fields, axis = 1) # Eliminamos columnas que no vamos a usar
        class_labels = {'excellent': 1, 'good': 2, 'acceptable': 3, 'spoiled': 4}
        data['class'] = data['class'].replace(class_labels)
...
class ModelController:
    ...
    def __init__(self):
        ...
        self.Random_model = joblib.load(self.Random_model_path)
        self.RN_model = joblib.load(self.RN_model_path)
    ...
    def predict_model_RN(self, data):
        drop_fields = ["minute"]
        target_feature = 'class'
        data = data.drop_duplicates() # Eliminamos duplicados
        data = data.drop(drop_fields, axis = 1) # Eliminamos columnas que no vamos a usar
        ...
        data['class'] = data['class'].replace(class_labels)
        X_data, y_variable = data.drop([target_feature], axis=1), data[target_feature]
        result = self.RN_model.predict(X_data)
